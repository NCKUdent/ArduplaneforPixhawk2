...
*/
static void custom_stabilize_roll()
{
    bool disable_integrator = false;
    if (control_mode == CUSTOM_STABILIZE && channel_roll->control_in != 0) {
        disable_integrator = true;
    }
    channel_roll->servo_out = rollController.custom_get_servo_out(custom_nav_roll_cd - ahrs.roll_sensor, 
                                                                  disable_integrator);
}
static void custom_stabilize_pitch()
{
    bool disable_integrator = false;
    if (control_mode == CUSTOM_STABILIZE && channel_pitch->control_in != 0) {
        disable_integrator = true;
    }
    channel_pitch->servo_out = pitchController.custom_get_servo_out(custom_nav_pitch_cd - ahrs.pitch_sensor,
                                                                    disable_integrator);
}
...
static void stabilize()
{
    ...
    } else if (control_mode == LATERAL) {
       lateral();
    } else if (control_mode == LONGITUDINAL) {
       longitudinal();
